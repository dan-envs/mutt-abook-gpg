...
**Note**: All Names added by Mutt or NeoMutt are prefixed automatically with
'#'. These are again removed when searching in Mutt or NeoMutt. This is needed
to find those entries by the `get_new_keys.sh` script. So if you enter an
address to your abook contacts manually, you **have** to prefix them with a '#'
char, otherwise they'll be deleted when running the `get_new_keys.sh` script
manually or from inside Mutt or NeoMutt. That is because the `abook
--mutt-query` command can't be used with an empty string as an parameter. So I
search addressess with the command `abook --mutt-query '#'`. Keep that in mind!
